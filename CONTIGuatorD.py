...
__credits__ = ["Diego Cesar Batista Mariano", "Felipe Luiz Pereira", "Lee Katz", "Florent Lassalle","Margaret Priest",
...
def ManualACT(name, sRef, sPC, sCrunch, outdir, mylog, options):
        	...
        	indextogetinfo = "product"
                if feat.type == "rRNA":
                    ftrack_set.add_feature(feat, color=colors.blue,
                                    border=colors.blue,label=False,
                                    label_size=2, label_angle=45,
                                    label_position="middle",
                ...
                if feat.type == "CDS":
                    ...
                    if cPlasmid > 0:
                        ftrack_set.add_feature(feat, color=colors.yellow,
                                    border=colors.yellow,label=False,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                    ...
                    if cTranspo > 0:
                        ftrack_set.add_feature(feat, color=colors.lightblue,
                                    border=colors.lightblue,label=False,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                indextogetinfo = "note"
    ...
    if options.lGenBankFilesBar != []:
        if os.path.isfile(options.lGenBankFilesBar[0]):
            ...
            print "GenBank create by barnnap not found."
    
    if titulo != "":
	#print gd_diagram
	gdt_features = gd_diagram.new_track(1, greytrack=False, scale_ticks=False)
	gds_features = gdt_features.new_set()    
	titulo = "  Reference: " + titulo + "  |  Date: " + strftime("%d %B %Y - %H:%M:%S") + "                          CAPTION | Above (reference) => yellow: plasmid; green: phage; light blue: transposon; blue: rRNA    |    Below (contigs) => contigs blue: no overlaps between contigs near; contigs orange: one overlap with some contig neighbor; red: two overlaps."
	feature = SeqFeature(FeatureLocation(25, 125), strand=+1)
	gds_features.add_feature(feature, name=titulo, label=True, label_size=10, label_angle=0)
